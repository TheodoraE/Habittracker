...
for (let i = 0; i < days.length; i++) {
    ...
    for (let j = 0; j < day.length; j++) {
        ...
        if (dayCount == currentDate - 1) {
            day[j].setAttribute("style", "color:aquablue");
        ...
        if (dayCount < daysInThisMonth) {
            ...
            day[j].innerHTML = "";
            day[j].setAttribute("style", "background-color:#fff0c1");
        }
    }
    rowCount++;
}
...
//    console.log(`${i +1}: ${chosenDay}`);
    ...
    if (chosenDay === "true") {
        ...
        chosenDayDiv.style.backgroundColor = "#fff0c1";
    }
}
...
for (let i = 0; i < currentDate; i++) {
    dayDivs[i].onclick = (e) => {
        ...
        if (localStorage.getItem(storageString) === "false"){
            selectedDate.style.backgroundColor = "aquamarine";
            localStorage.setItem(storageString, true);
            daysCompleted++;
        } else if (localStorage.getItem(storageString) === "true"){
            selectedDate.style.backgroundColor = "#fff0c1";
...
resetButton.onclick = () => {
    for (let i = 0; i < dayCount; i++) {
        ...
        curDay.style.backgroundColor = "#fff0c1";
    }
    daysCompleted = 0;
    totalDays.innerHTML = `${daysCompleted}/${daysInThisMonth}`;
    habitTitle.innerHTML = "Click to set your habit";
